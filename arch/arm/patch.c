...
static int patch_check_insn(insn_kind_t kind, insn_t insn) {
    ...
    switch (kind) {
        ...
        case INSN_KIND_THUMB2:
            ...
            return is_thumb2_halfword(thumb2_first_halfowrd(insn));
...
bool patch_insn(thread_t * thread, address_t address, insn_kind_t kind, insn_t insn) {
    ...
    //        (void *) address, insn, arm_disassemble_extended(insn, kind, address));

    if (kind == INSN_KIND_THUMB) {
        uint16_t insn16 = (uint16_t) insn;
        return (mem_write(thread, address, 2, &insn16) == 2);
    } else {
        if (kind == INSN_KIND_THUMB2)
            insn = thumb2_swap_halfwords(insn);
...
void patch_relative_jump(thread_t * thread, address_t insn_address, address_t dest_address, insn_kind_t kind) {
    patch_insn(thread, insn_address, kind, arm_get_relative_jump_insn(kind, insn_address, dest_address));
}
...
bool patch_read_insn_detect_kind(thread_t * thread, address_t address, insn_t * insn, insn_kind_t * kind) {
    ...
    if (*kind == INSN_KIND_ARM) {
        ...
        if (!patch_read_insn(thread, address, insn, INSN_KIND_THUMB))
            return false;
            
        if (is_thumb2_halfword(*insn)) {
            *kind = INSN_KIND_THUMB2;
            return patch_read_insn(thread, address, insn, INSN_KIND_THUMB2);
        } else {
